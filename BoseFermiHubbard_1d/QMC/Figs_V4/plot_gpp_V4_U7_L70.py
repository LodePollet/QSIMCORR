...
data = np.loadtxt("stat_dens_V4_U7_L70.out")
xpp = data[:,0]
gpp = data[:,15]
gpp_err = data[:,16]
gph = data[:,13]
...
cdw_data = np.loadtxt("stat_cdwsdw_V4_U7_L70.out")
xcdw = cdw_data[:,0]
cdw = cdw_data[:,1]
...
def fexp(x,a,b):
  #return(a * np.exp(-b*np.sin(np.pi*x/L)))
  return(a * np.exp(-b*x))
...
popt_ph, pcov_ph = curve_fit(fexp, xdata=xpp[6:L//2-8], ydata=gph[6:L//2-8],sigma=gph_err[6:L//2-8])
...
plt.figure()
plt.errorbar(np.log(xpp[1:L//2]), np.log(np.abs(gpp[1:L//2])), yerr = gpp_err[1:L//2] / gpp[1:L//2] * np.abs(np.log(gpp_err[1:L//2] + 1e-12)), fmt = "bo", label=r"$\mathcal{G}^{pp}$")
plt.errorbar(np.log(xpp[1:L//2]), np.log(np.abs(gph[1:L//2])), yerr = gph_err[1:L//2] / gph[1:L//2] * np.abs(np.log(gph_err[1:L//2] + 1e-12)), fmt = "gx", label=r"$\mathcal{G}^{ph}$")
plt.plot(np.log(xpp[5:L//2]), np.log(np.abs(g(xpp[5:L//2], *popt_pp))), "r-", label=r"$fit, \eta_{pp} \approx %2.2f$"  %(popt_pp[0])) 
plt.plot(np.log(xpp[6:L//2-5]), np.log(np.abs(fexp(xpp[6:L//2-5], *popt_ph))), "m-", label=r"fit, exp") 
plt.legend(loc="best", fontsize=16)
plt.xlabel(r"$\ln x$", fontdict=font1)
plt.ylabel(r"$\ln \mathcal{G}^{pp}(x), \mathcal{G}^{ph}(x)$", fontdict=font1)
plt.xticks(fontsize=16)
plt.yticks(fontsize=16)
plt.savefig("fig_pp_V4_U7_L70.pdf", format="pdf", bbox_inches="tight")
...
plt.xticks(fontsize=16)
plt.yticks(fontsize=16)
plt.savefig("fig_cdw_V4_U7_L70.pdf", format="pdf", bbox_inches="tight")
