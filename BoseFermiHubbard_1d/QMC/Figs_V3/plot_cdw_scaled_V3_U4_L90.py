...
data = np.loadtxt("stat_structfact_V3_U4_L90.out")
xsf_cdw = data[:,0]
...
plt.plot(xfit, popt_sf_cdw[0]*np.ones_like(xfit), "r--", label = r"$fit, \eta_{\rm CDW} \approx %2.2f$" %(popt_sf_cdw[0]))
...
plt.savefig("fig_sf_cdw_scaled_V3_U4_L90.pdf", format="pdf", bbox_inches="tight")
