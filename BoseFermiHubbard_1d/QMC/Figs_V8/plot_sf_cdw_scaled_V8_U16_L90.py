...
data = np.loadtxt("stat_structfact_V8_U16_L90.out")
#data = np.loadtxt("stat_structfact.out")
xsf_cdw = data[:,0]
...
popt_sf_cdw, pcov_sf_cdw = curve_fit( linear, xdata=xsf_cdw[1:4], ydata=ysf_cdw[0:3], sigma = ysf_cdw_err[0:3])
...
plt.plot(xfit, linear(xfit,*popt_sf_cdw), "r--", label = r"$fit, \eta_{\rm CDW} \approx %2.1f$" %(popt_sf_cdw[0]))
plt.plot(xfit, linear(xfit,*popt_sf_sdw), "c--", label = r"$fit, \eta_{\rm SDW} \approx %2.1f$" %(popt_sf_sdw[0]))
plt.legend(loc="best", fontsize=14)
plt.xlabel(r"$k$", fontdict=font1)
plt.xlim([0, np.pi * 2])
plt.ylabel(r"$ \frac{ \pi S^{\rm CDW}(k) }{k} $", fontdict=font1)
plt.xticks(fontsize=16)
plt.yticks(fontsize=16)
plt.savefig("fig_sf_cdw_scaled_V8_U16_L90.pdf", format="pdf", bbox_inches="tight")
