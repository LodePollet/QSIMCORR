...
cdw_data = np.loadtxt("stat_cdwsdw_V8_U16_L90.out")
#cdw_data = np.loadtxt("stat_cdwsdw.out")
xcdw = cdw_data[:,0]
L = xcdw.size
print(L)
cdw = cdw_data[:,1]
...
#plt.savefig("fig_pp_V8_U16_L90.pdf", format="pdf")

xcdw_fit = np.linspace(0.5,L/2,num=(L//2-1)*21)

plt.figure()
plt.errorbar(xcdw, cdw, yerr = cdw_err, fmt = 'b.')
plt.plot(xcdw_fit, f(xcdw_fit, *popt_cdw), "r--", label = r"$fit, {\eta}_{cdw} \approx  %2.1f$" %(popt_cdw[0]))
plt.plot(xcdw_fit, fconst(xcdw_fit, *popt_const_cdw), "c--", label = r"$fit, C \approx  %3.2f$" %(popt_const_cdw[2]))
...
plt.savefig("fig_cdw_V8_U16_L90.pdf", format="pdf", bbox_inches="tight")
