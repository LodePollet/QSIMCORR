...
a=5
#fig, ax = plt.subplots(2, 3,figsize=(a*1.62,a*2.28))
...
#disorder values
p_values=[0.050,0.100,0.148]
#reweighting parameters
#if 4 is included take these indices
#indices_list=[[32,28,33,27],[31,30,30,30],[31,31,29,32],[31,31,29,32],[31,31,28,31]]
#delta_k_list=[[-0.005,0.005,0.003,0.003],[0.004,-0.002,0.004,0.00],[0.0,-0.02,0.000,0.002],[0.002,-0.028,-0.002,-0.004],[0.0,-0.043,0.01,-0.002]]
#if 4 is omited:
indices_list=[[30,31,26],[30,31,27],[28,29,25],]
delta_k_list=[[0.004,0.0,0.002],[0.003,0.003,0.009],[0.004,0.,0.008]]
...
for p_num,p in enumerate(p_values):
    ...
    for L_counter,L in enumerate(sizes):
        ...
        temp_axis=np.load("PIM/p=%0.03f/L%03d/Temperature_axis.npy"%(p,L))
        x_axis=np.load('PIM/p=%0.03f/one_pure_L%03d_%s_%s_hdf5.npy'%(p,L,hist_obs_name[0],index))
        y_values=np.load('PIM/p=%0.03f/two_pure_L%03d_%s_%s_hdf5.npy'%(p,L,hist_obs_name[0],index))
    ...
    #ax[p_num].text(0.24, 0.3,r"$T$", horizontalalignment='center', verticalalignment='center', transform = ax[p_num].transAxes,fontsize=fontsize_legend)
    #change location of the legend here as bbox_to_anchor=...

    ax[p_num].legend(bbox_to_anchor=(-0.08, 0.0, 0.1,0.5), loc="lower left",handletextpad=-0.4,labelspacing=0.1,fontsize=fontsize_legend, frameon=False)
    #ax[p_num].legend(bbox_to_anchor=(-0.1, -0.05,0.1,0.5),loc="lower left",frameon=False,handletextpad=-0.7,labelspacing=0.1,fontsize=fontsize_legend)
...
fig.savefig("fig_P_B_PIM.pdf",format='pdf',bbox_inches='tight', dpi=300)
